...
\begin{exercise}[]
  ...
  \begin{subex}
    ...
    \mid C)$ 
    %using only the fact that for any event $E \in \mathcal{A}$ $\mathbb{P}(E)
    % + \mathbb{P}(\bar{E}) = 1$\footnote{Also called the {\em sum rule}, cp.
    % Definition 2.2.} and Definition 2.5 (conditional probability).
  \end{subex}
\end{exercise}
